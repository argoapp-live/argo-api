  ...
  async (req, res) => {
    const userProfileModel: IUserModel = await AuthService.findProfileOrCreate({
      providerProfile: {
        ...
        email: req.user.profile.emails?.filter(
          (email: any) => email.primary || email.primary === undefined
        )[0].value,
      },
    ...
    res.redirect(
      `${config.frontendApp.HOST_ADDRESS}/#/callback/github?token=${token}`
    );
  }
);
...
router.get('/github/app', async (req, res) => {
  ...
  const id = Types.ObjectId(deserializedToken.session_id);
