...
export async function triggerWebHook(
  ...
  try {
    ...
    const webHook: IWebHook = await WebHookService.findOne({
      projectId,
      branch,
    });
    ...
    const responseObj: any = await DeploymentComponent.deploy(
      project.githubUrl,
...
export async function update(
  ...
  try {
    ...
    const webHook: IWebHook = await WebHookService.update(
      req.params.id,
      req.body
    );
