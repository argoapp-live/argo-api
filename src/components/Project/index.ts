...
export async function findOne(
  ...
  try {
    ...
    if (project) {
      ...
      project._doc.subdomains = domains.filter(
        (domain) => domain.type === "subdomain"
      );
      project._doc.handshakeDomains = domains.filter(
        (domain) => domain.type === "handshake-domain"
      );
...
export async function create(
  ...
  try {
    const repository: IProject = await ProjectService.insert(
      req.body,
...
export async function findOneAndUpdate(
  ...
  try {
    const repository: boolean = await ProjectService.findOneAndUpdate(
      req.params.id,
      req.body
    );
    res.status(200).json({
...
export async function updateEnv(
  ...
  try {
    const updatedProject: boolean = await ProjectService.updateEnv(
      req.params.id,
      req.body
    );
    res.status(200).json({
