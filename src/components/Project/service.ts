...
//         try {
//             const repository: IProject = await ProjectModel.findOne({
...
//      * @memberof UserService
//      */
//     async findRepoByNameAndBranch(repoName: string, branchName: string): Promise<IProject> {
//         try {
//             const repository: IProject = await ProjectModel.findOne({
...
//      * @memberof UserService
//      */
//     async insert(repository: IProject, organizationId: string): Promise<IProject> {
//         try {
...
//             }
//             // const repo: IProject = organization.repositories.find((r: IProject) => r.name === repository.name);
//             // if (repo !== undefined) {
...
//         const repo: IProject = await ProjectModel.findOne({ url: githubUrl, orgId: Types.ObjectId(orgId) });
...
//         const repository: IProject = await ProjectModel.create({
//             url: githubUrl,
...
//                 '_id': Types.ObjectId(id)
//             };
//             var repo = await ProjectModel.findOne(filter);
...
//                 }
//             }
//             await ProjectModel.findOneAndUpdate(filter, updatCondition);
//             return true;
...
//             };
//             await ProjectModel.updateOne(filter, { $pull: { subDomains: { _id: Types.ObjectId(id) } } });
//             return true;
...
//                 '_id': Types.ObjectId(repositoryId)
//             };
//             await ProjectModel.updateOne(filter, { $pull: { domains: { _id: Types.ObjectId(id) } } });
//             return true;
...
//     async AddToProxy(repo: IProject, txId: string, depId: string): Promise<any> {
