...
function _populateProject(project: any, domains: Array<IDomain>): any {
  ...
  project._doc.handshakeDomains = projectDomains.filter(
    (domain) => domain.type === "handshake-domain"
  );
  project._doc.handshakeSubdomains = projectDomains.filter(
...
export async function deleteUser(
  ...
  try {
    ...
    const deletedUser = await UserService.deleteOrganisation(req.params.id, req.body.userId);
    res.status(200).json(organization);
  } catch (error) {
    next(new HttpError(error.message.status, error.message));
  }
