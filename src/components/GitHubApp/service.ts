...
if (config.githubApp.PEM_CONTENT_BASE64) {
  ...
  const gitPrivateKeyPath: string = path.join(
    __dirname,
...
const GithubAppService: IGitHubAppTokenService = {
  ...
  async findByInstallationAndUserId(
    id: Types.ObjectId,
  ...
  async getFullGithubUrlAndFolderName(
    branch: string,
    installationId: number,
    owner: string,
    folderName: string
  ): Promise<string> {
    let installationToken = await GithubAppService.createInstallationToken(
      installationId
    );
  ...
  async getInstallationRepos(id: string, installationId: any): Promise<any> {
    const getUserToken = await GitHubAppTokenModel.findOne({
      argoUserId: Types.ObjectId(id),
    });
  ...
  async getLatestCommitInfo(
    ...
    try {
      ...
      return {
        id: res.data[0].commit.url.split("commits/")[1],
