...
import { Nft, Vendor } from "@argoapp/nft-js";
...
export async function getMetadataUrl(
  ...
  try {
    ...
    const signer = Wallet.fromMnemonic(config.nft.MNEMONIC).connect(
      httpProvider
    );
    const vendor = new Vendor(
      ...
      undefined,
      undefined,
      config.nft.NFT_SUBGRAPH
    );
    const nftLib = new Nft(vendor);
    ...
    const uploaderVendor: uploaderLib.Vendor = new uploaderLib.Vendor(
      Buffer.from(config.arweave.PRIVATE_KEY, "base64").toString()
    );
    const uploader: uploaderLib.Uploader = new uploaderLib.Uploader(
      uploaderVendor
    );
    const tx = await uploader.uploadMetadataToArweave(
